...
REGION="us-east-1"
API_NAME="dev-automation-api"
POOL_NAME="automation-platform-users"
CLIENT_NAME="automation-platform-web"
AUTHORIZER_NAME="automation-platform-cognito-auth"

echo "=== Automation Platform Cognito Authorizer Setup ==="
echo ""
...
if [ -z "$API_ID" ] || [ "$API_ID" == "None" ]; then
  echo "Error: API Gateway '$API_NAME' not found. Deploy the CDK stack first."
  exit 1
fi
...
# Workflow mutations
add_auth_to_route "POST /workflows"
add_auth_to_route "PUT /workflows/{workflow_id}"
add_auth_to_route "DELETE /workflows/{workflow_id}"
add_auth_to_route "PATCH /workflows/{workflow_id}/enabled"

# Workflow execution
add_auth_to_route "POST /workflows/{workflow_id}/execute"

# Secrets management
add_auth_to_route "GET /secrets"
add_auth_to_route "POST /secrets"
add_auth_to_route "DELETE /secrets/{name}"
...
for ROUTE_KEY in "${PUBLIC_ROUTES[@]}"; do
  ROUTE_ID=$(aws apigatewayv2 get-routes \
    --api-id $API_ID \
    --region $REGION \
    --query "Items[?RouteKey=='$ROUTE_KEY'].RouteId" --output text)
  ...
  if [ -n "$ROUTE_ID" ] && [ "$ROUTE_ID" != "None" ]; then
    ...
    echo "  $ROUTE_KEY: Not found (may need CDK deploy)"
  fi
done
...
echo "  - POST /workflows"
echo "  - PUT /workflows/{workflow_id}"
echo "  - DELETE /workflows/{workflow_id}"
echo "  - PATCH /workflows/{workflow_id}/enabled"
echo "  - POST /workflows/{workflow_id}/execute"
echo "  - GET /secrets"
echo "  - POST /secrets"
echo "  - DELETE /secrets/{name}"
echo ""
echo "Public Routes (no auth):"
echo "  - GET /health"
echo "  - GET /workflows"
echo "  - GET /workflows/{workflow_id}"
echo "  - GET /workflows/{workflow_id}/executions"
echo "  - GET /workflows/{workflow_id}/executions/{execution_id}"
echo "  - POST /webhook/{workflow_id}"
echo ""
echo "============================================"
echo ""
echo "Test protected route without token (should return 401):"
echo "  curl -X POST ${API_ENDPOINT}/workflows -H 'Content-Type: application/json' -d '{\"name\":\"Test\"}'"
echo ""
echo "Test public route (should work):"
echo "  curl ${API_ENDPOINT}/workflows"
